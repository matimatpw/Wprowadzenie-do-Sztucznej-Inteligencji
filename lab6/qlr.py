...
def boltzmann_exploration(q_values, temperature):
    probabilities = np.exp(q_values / temperature) / np.sum(
        np.exp(q_values / temperature), axis=0
    )
...
def q_learning(data: data):
    ...
    for episode in range(data.num_episodes):
        ...
        while not done and steps < data.steps_limit:
            ...
            q_table[state, action] = (1 - data.learning_rate) * q_table[
                state, action
            ] + data.learning_rate * (
                reward + data.importance_future * np.max(q_table[next_state, :])
            )
            state = next_state
...
def evaluate_policy(data: data, q_table):
    ...
    for episode in range(data.evaluate_episodes):
        if episode % 100 == 0 and episode != 0:
            counter += 1
            avg_steps.append(counter)
            avg_rewards.append(counter_rewards / 100)
            counter_rewards = 0
    ...
    return average_reward, episode_rewards, episode_steps, avg_steps, avg_rewards
...
def main():
    ...
    for learning_rate in input_data.learning_rates:
        for temperature in input_data.temperatures:
            ...
            (
                avg_reward_epsilon,
                episode_rewards_greedy,
                episode_steps_greedy,
                avg_steps,
                avg_rewards_eps,
            ) = evaluate_policy(input_data, q_table_epsilon)
            print(
                f"Epsilon-Greedy - Learning Rate: {learning_rate}, Average Reward: {avg_reward_epsilon}"
            )

            q_table_boltzmann = q_learning(input_data)
            (
                avg_reward_boltzmann,
                episode_rewards_boltz,
                episode_steps_bolts,
                avg_steps_bolt,
                avg_rewards_bolt,
            ) = evaluate_policy(input_data, q_table_boltzmann)
            print(
                f"Boltzmann - Temperature: {temperature}, Average Reward: {avg_reward_boltzmann}"
            )

            plt.plot(
                avg_steps,
                avg_rewards_eps,
                label=f"Epsilon-Greedy avg",
                color="red",
                linestyle="-",
                alpha=0.8,
            )
            ...
            plt.title(
                f"Episode Rewards Over Episodes - lr_{learning_rate} & T_{temperature}"
            )
